...
from GLOBAL.GLOB import WIDTH, HEIGHT, WINDOW


"""
==========
класс Bullet: воспроизведение звука выстрела, работа со снарядами
----------
init(): создание переменных-характеристик снаряда, вызывается в Tank
----------
update(): обновления объектов, вызывается в PLAYING
----------
draw(): отображает: снаряд, "след снаряда", "огненную вспышку"; вызывается в PLAYING
==========
"""
...
class Bullet:
	...
	def draw(
		...
		if not self.flag:
			pygame.draw.circle(screen, (255, 165, 0), (self.parent_x - self.vector_x, self.parent_y - self.vector_y), 3)
			pygame.draw.circle(
